...
//void Graphics::DrawSprite(int x, int y, Rect rect, const Rect& clipping, const Surface& s, E effect)
//{
...
void Graphics::DrawLine(Vec2f p0, Vec2f p1, Color c)
{
	float m = 0.0f;
	if (p1.x != p0.x)
	{
...
void Graphics::DrawClosedPolyline(const std::vector<Vec2f>& verts, Color c)
{
...
void Graphics::DrawRect(float x, float y,Vec2f min, Vec2f max,Rect clipping, Color c)
{
	float width = max.x - min.x;
	float height = max.y - min.y;

	if (x + width > clipping.right)
	{
		max.x = clipping.right - x;
	}
	if (x < 0)
	{
		min.x = clipping.left - x;
	}
